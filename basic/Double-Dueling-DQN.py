...
annealing_steps = 5000#10000. #How many steps of training to reduce startE to endE.
num_episodes = 2000#10000 #How many episodes of game environment to train network with.
pre_train_steps = 2000#10000 #How many steps of random actions before training begins.
...
class Qnetwork():
	def __init__(self,h_size):
		...
		self.scalarInput =  tf.placeholder(shape=[None,image_side_size * image_side_size * 3],dtype=tf.float32)
		self.imageIn = tf.reshape(self.scalarInput,shape=[-1,image_side_size,image_side_size,3])
...
class experience_buffer():
	...
	def sample(self,size):
		return np.reshape(np.array(random.sample(self.buffer,size)),[size,env.actions + 1])


# This is a simple function to resize our game frames.

def processState(states):
	return np.reshape(states,[image_side_size * image_side_size * 3])
...
with tf.Session() as sess:
	...
	for i in range(num_episodes):
		...
		while j < max_epLength: #If the agent takes longer than 200 moves to reach either of the blocks, end the trial.
			...
			s1 = processState(s1)
			total_steps += 1
			episodeBuffer.add(np.reshape(np.array([s,a,r,s1,d]),[1, env.actions + 1])) #Save the experience to our episode buffer.
