...
class DQNNTwoDiodeLaser(NN):
	def __init__(self, hyperparameters, environment, *args, **kwargs):
		...
		fully_conn2 = slim.fully_connected(inputs=self.scalarInput,num_outputs=8,\
		  biases_initializer=None,activation_fn=tf.nn.relu,weights_initializer=tf.ones_initializer())
		fully_conn3 = slim.fully_connected(inputs=fully_conn2,num_outputs=8,\
		  biases_initializer=None,activation_fn=tf.nn.relu,weights_initializer=tf.ones_initializer())
		fully_conn4 = slim.fully_connected(inputs=fully_conn3,num_outputs=8,\
		  biases_initializer=None,activation_fn=tf.nn.relu,weights_initializer=tf.ones_initializer())


		streamA, streamV = tf.split(fully_conn4, 2, 1)
